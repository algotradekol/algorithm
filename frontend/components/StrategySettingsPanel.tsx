  ...
  return (
    <div className="mt-5 rounded border border-[#1f2937] bg-[#111827] p-3">
      <label className="flex gap-3">
        ...
        <span><span className="text-sm font-semibold text-gray-100">Test Schedule</span><span className="mt-1 block text-xs text-gray-500">Uses a future intraday candle for a paper-only pipeline check. Turn this off to restore the 09:15 signal-candle production schedule.</span></span>
      </label>
      {enabled && <label className="mt-3 block"><div className="label">Test Window Start (IST)</div><input type="time" value={settings.test_candle_time || '11:10'} onChange={(e) => setSettings({ ...settings, test_candle_time: e.target.value })} className="control mt-1" /><p className="mt-1 text-xs text-[#f59e0b]">The strategy collects the configured signal candle from this time, then enters on the next candle open. It still compares against the previous-day close, so this is a systems test, not a valid opening-gap trade signal.</p></label>}
    </div>
  );
}
