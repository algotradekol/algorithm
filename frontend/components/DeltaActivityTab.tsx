...
export default function DeltaActivityTab({ enabledTimeframes, asset = 'gold', mode = 'paper' }: { enabledTimeframes: number[]; asset?: DeltaAsset; mode?: 'paper' | 'live' }) {
  ...
  return <section className="space-y-4">
    ...
    <p className={`rounded border p-3 text-sm ${source === 'paper' ? 'border-[#3b82f6]/40 text-[#93c5fd]' : 'border-[#f59e0b]/40 text-[#fbbf24]'}`}>
      {source === 'paper' ? 'PAPER: stops and targets are virtual engine protection, not exchange orders.' : 'LIVE ACCOUNT / READ ONLY: the selected metal on the configured Delta account. This does not enable live strategies or permit placing, editing or cancelling orders.'}
    </p>
    ...
    <ActivityRows key={`${asset}-${source}-${minutes}-${kind}`} asset={asset} source={source} minutes={minutes} kind={kind} />
  </section>;
}
...
function ActivityRows({ source, minutes, kind, asset }: { source: string; minutes: number; kind: keyof typeof views; asset: DeltaAsset }) {
  ...
  }, [asset, source, minutes, kind, cursors, refresh]);
  ...
  return <>
    ...
    <p className="text-xs text-gray-500">1 displayed lot = 1 exchange quantity unit. Missing margin or P&amp;L is --, not zero. Paper margin uses the saved strategy leverage; live margin is shown only when Delta supplies it. {source === 'live' ? 'Order history is not a realized P&L ledger.' : ''}</p>
