...
type Settings = { silver_breakout_points: number; sl_points: number; target_points: number; tsl_activate_points: number; tsl_buffer_points: number; tsl_profit_step_points: number; tsl_lock_step_points: number; silver_lots: number; size_mode: 'lots' | 'pax'; pax_size: number; leverage: number; post_exit_cooldown_minutes: number; exit_mode: string; strategy_version: string };
type Trade = { id: string; side: string; qty: number; entry_time: string; entry_price: number; exit_time?: string; exit_price?: number; exit_reason?: string; initial_sl: number; sl_price: number; target_price: number; estimated_entry_margin?: number; margin_inr?: number; net_pnl?: number; gross_pnl?: number; fees?: number; pnl_inr?: number; unrealized_pnl?: number; signal_snapshot: { setup_time?: string; setup_close?: number; trigger_level?: number; silver_breakeven?: { armed: boolean; activation_price: number }; delta_three_candle_tsl?: { events?: unknown[] }; delta_ladder_tsl?: { events?: unknown[] } } };
...
const initial: Settings = { silver_breakout_points: 3, sl_points: 15, target_points: 50, tsl_activate_points: 15, tsl_buffer_points: 3, tsl_profit_step_points: 15, tsl_lock_step_points: 15, silver_lots: 1, size_mode: 'lots', pax_size: 0.001, leverage: 50, post_exit_cooldown_minutes: 5, exit_mode: 'fixed_target_sl', strategy_version: 'paxg_ema_volume_v1' };
const initialFor = (asset: DeltaAsset): Settings => asset === 'silver'
  ? { ...initial, silver_breakout_points: 0.10, sl_points: 0.30, target_points: 1.0, tsl_activate_points: 0.30, exit_mode: 'target_to_breakeven_sl', strategy_version: 'silver_micro_delta_usd_v1' }
  : initial;
  ...
  return <section className="space-y-4">
    ...
    <form onSubmit={run} className="rounded border border-[#1f2937] bg-[#111827] p-4"><fieldset disabled={busy || !enabledTimeframes.length} className="space-y-4 disabled:opacity-60">
      <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
        ...
        <label className="text-xs text-gray-400">Exit mode<select className={`${control} mt-1`} value={settings.exit_mode} onChange={e => setSettings({ ...settings, exit_mode: e.target.value })}><option value="fixed_target_sl">Fixed target + SL</option><option value="target_to_breakeven_sl">Target + breakeven SL</option>{asset === 'gold' && <option value="three_candle_tsl">Three-Candle TSL</option>}{asset === 'gold' && <option value="continuous_ladder_tsl">Continuous Ladder TSL</option>}</select></label>
        {([['silver_breakout_points', 'Breakout offset'], ['sl_points', 'Initial SL points'], ['target_points', 'Final target points'], ['tsl_activate_points', settings.exit_mode === 'continuous_ladder_tsl' ? 'Target initial / breakeven at' : 'TSL activation points'], ['tsl_buffer_points', 'TSL buffer points'], ['tsl_profit_step_points', 'Target trailing step'], ['tsl_lock_step_points', 'SL trailing step']] as const).filter(([key]) => (key !== 'tsl_activate_points' || settings.exit_mode === 'target_to_breakeven_sl' || settings.exit_mode === 'continuous_ladder_tsl') && (key !== 'tsl_buffer_points' || settings.exit_mode === 'three_candle_tsl') && (key !== 'tsl_profit_step_points' || settings.exit_mode === 'continuous_ladder_tsl') && (key !== 'tsl_lock_step_points' || settings.exit_mode === 'continuous_ladder_tsl')).map(([key, label]) => <label key={key} className="text-xs text-gray-400">{label}<input className={`${control} mt-1`} type="number" required min={.000001} step="any" value={settings[key]} onChange={e => setSettings({ ...settings, [key]: Number(e.target.value) })} /></label>)}
        <label className="text-xs text-gray-400">Size mode<select className={`${control} mt-1`} value={settings.size_mode} onChange={e => setSettings({ ...settings, size_mode: e.target.value as Settings['size_mode'] })}><option value="lots">By lots</option>{asset === 'gold' && <option value="pax">By PAXG amount</option>}</select></label>
        {settings.size_mode === 'pax' && asset === 'gold'
          ? <label className="text-xs text-gray-400">PAXG per trade<input className={`${control} mt-1`} type="number" required min={.000001} step="any" value={settings.pax_size} onChange={e => setSettings({ ...settings, pax_size: Number(e.target.value) })} /></label>
          : <label className="text-xs text-gray-400">Lots per trade<input className={`${control} mt-1`} type="number" required min={1} step={1} value={settings.silver_lots} onChange={e => setSettings({ ...settings, silver_lots: Number(e.target.value) })} /></label>}
        <label className="text-xs text-gray-400">Leverage (x)<input className={`${control} mt-1`} type="number" required min={.000001} step="any" value={settings.leverage} onChange={e => setSettings({ ...settings, leverage: Number(e.target.value) })} /></label>
