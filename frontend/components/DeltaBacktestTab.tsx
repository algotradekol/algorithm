  ...
  return <section className="space-y-4">
    ...
    <form onSubmit={run} className="rounded border border-[#1f2937] bg-[#111827] p-4"><fieldset disabled={busy || !enabledTimeframes.length} className="space-y-4 disabled:opacity-60">
      <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
        ...
        <label className="text-xs text-gray-400">Post-exit rest<select className={`${control} mt-1`} value={settings.post_exit_cooldown_minutes} onChange={e => setSettings({ ...settings, post_exit_cooldown_minutes: Number(e.target.value) })}><option value={5}>5 minutes</option><option value={15}>15 minutes</option><option value={30}>30 minutes</option><option value={60}>1 hour</option><option value={240}>4 hours</option><option value={720}>12 hours</option></select></label>
      </div>
      <p className="text-xs text-gray-400">{enabledTimeframes.length ? 'Maximum 31 days; today uses completed minutes only. One lot equals one exchange quantity unit. Risk distances use the quoted price, not INR. Missing candles fail the run instead of inventing prices.' : 'No Delta strategy timeframe is enabled for backtesting in this deployment.'}</p>
