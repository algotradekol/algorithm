...
export const api = {
  ...
  setupHistory: (
    ...
    days = 30,
    limit = 100,
    options?: { currentSessionOnly?: boolean; liveOnly?: boolean },
  ) => {
    const currentSessionOnly = options?.currentSessionOnly ? '&current_session_only=true' : '';
    const liveOnly = options?.liveOnly ? '&live_only=true' : '';
    return authedFetch(
      `/api/algo/${algoId}/setup-history?side=${encodeURIComponent(side)}&days=${days}&limit=${limit}${currentSessionOnly}${liveOnly}`,
    );
  },
...
export function deltaApi(asset: DeltaAsset = 'gold', mode: TradingMode = 'paper') {
  ...
  deltaPause: (minutes: number, durationMinutes: number, afterTrade = false) => fetchAsset(`/api/delta/${minutes}/pause`, { method: 'POST', body: JSON.stringify({ duration_minutes: durationMinutes, after_trade: afterTrade }) }),
  // payload keys: position_id, sl_price, target_price, expected_sl, expected_target,
  // and optionally exit_mode ('fixed_target_sl' | 'target_to_breakeven_sl' | 'three_candle_tsl' | 'continuous_ladder_tsl')
